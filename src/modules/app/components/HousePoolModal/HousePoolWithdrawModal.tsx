    ...
    const getdata = async () => {
            try {
                ...
                if (userAddress) {
                    ...
                    setDepositList(depositsArray.map((item: any, i: number) => {
                        console.log("timer"+i, parseFloat(item.DepositTime), parseFloat(releaseTime), new Date().getTime()/1000);
                        
                        return ({ ...item, releaseTime: releaseTime, presentBalance: mypresentBalances[i], 
                            pendingRewards: pendingRewards[i], 
                            lockedTimePeriod: (parseFloat(item.DepositTime)+parseFloat(releaseTime))-(new Date().getTime()/1000), 
                            isUnlocked:(parseFloat(item.DepositTime)+parseFloat(releaseTime))-(new Date().getTime()/1000)<=0,
                            index: i })//   10 + (10*i)
                    }));
                }
                setLoading(false);
